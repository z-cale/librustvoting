...
// ---------------------------------------------------------------------------
// HasActiveOrTallyingRound
// ---------------------------------------------------------------------------

func (s *KeeperTestSuite) TestHasActiveOrTallyingRound_Empty() {
	s.SetupTest()
	kv := s.keeper.OpenKVStore(s.ctx)

	// No rounds stored — should return false.
	has, err := s.keeper.HasActiveOrTallyingRound(kv)
	s.Require().NoError(err)
	s.Require().False(has, "empty store should have no active/tallying rounds")
}

func (s *KeeperTestSuite) TestHasActiveOrTallyingRound_OnlyFinalized() {
	s.SetupTest()
	kv := s.keeper.OpenKVStore(s.ctx)
	...
	has, err := s.keeper.HasActiveOrTallyingRound(kv)
	s.Require().NoError(err)
	s.Require().False(has, "only finalized rounds should return false")
}

func (s *KeeperTestSuite) TestHasActiveOrTallyingRound_ActiveRound() {
	s.SetupTest()
	kv := s.keeper.OpenKVStore(s.ctx)
	...
	has, err := s.keeper.HasActiveOrTallyingRound(kv)
	s.Require().NoError(err)
	s.Require().True(has, "should detect ACTIVE round")
}

func (s *KeeperTestSuite) TestHasActiveOrTallyingRound_TallyingRound() {
	s.SetupTest()
	kv := s.keeper.OpenKVStore(s.ctx)
	...
	has, err := s.keeper.HasActiveOrTallyingRound(kv)
	s.Require().NoError(err)
	s.Require().True(has, "should detect TALLYING round")
}

func (s *KeeperTestSuite) TestHasActiveOrTallyingRound_MixedStatuses() {
	s.SetupTest()
	kv := s.keeper.OpenKVStore(s.ctx)

	// One finalized, one active.
	s.Require().NoError(s.keeper.SetVoteRound(kv, &types.VoteRound{
		VoteRoundId: bytes.Repeat([]byte{0x01}, 32),
		VoteEndTime: expiredEndTime,
		Status:      types.SessionStatus_SESSION_STATUS_FINALIZED,
	...
	s.Require().NoError(s.keeper.SetVoteRound(kv, &types.VoteRound{
		VoteRoundId: bytes.Repeat([]byte{0x02}, 32),
		VoteEndTime: activeEndTime,
		Status:      types.SessionStatus_SESSION_STATUS_ACTIVE,
	}))
	...
	has, err := s.keeper.HasActiveOrTallyingRound(kv)
	s.Require().NoError(err)
	s.Require().True(has, "should detect the ACTIVE round among mixed statuses")
}
